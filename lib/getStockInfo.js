...
export async function GetStockInfoFugle(stock_number, type, from=null, to=null) {
	try {
		...
		if (from === null || to === null) {
			response = await fetch(`/api/fugle/marketdata/candles?symbol=${stock_number}&type=${type}`);
		} else {
			response = await fetch(`/api/fugle/marketdata/candles?symbol=${stock_number}&type=${type}&from=${from}&to=${to}`);
		}
...
export async function GetQuotesFugle(stock_number) {
	try {
		const response = await fetch(`/api/fugle/marketdata/quotes?symbol=${stock_number}`);
		// Check if the request was successful (status code 200)
		if (!response.ok) {
			throw new Error(`Failed to fetch data. Status: ${response.status}`);
		}
